...
// Funciones para calcular lunes y domingo de cualquier fecha
const getMonday = (date: Date) => {
  const d = dayjs(date).tz("America/Lima");
  const day = d.day();
  // Si es domingo (0), restar 6 días, si no, restar (day-1)
...
const getSunday = (date: Date) => {
  const d = dayjs(date).tz("America/Lima");
  const day = d.day();
  // Si es domingo (0), es el mismo día, si no, sumar (7-day) días
  return d.add(day === 0 ? 0 : 7 - day, "day").endOf("day").toDate();
};
...
export default function WeeklyStatsPage() {
  ...
  // Estado para el rango de la semana (calculado automáticamente de lunes a domingo)
  const [range, setRange] = useState<{ from: Date; to: Date }>(() => {
    const today = dayjs().tz("America/Lima").toDate();
  ...
  // Función para manejar cambio de fecha y calcular semana automáticamente (lunes a domingo)
  const handleDateChange = (date?: Date) => {
    if (!date) return;
    
    setSelectedDate(date);
    
    // Calcular automáticamente el rango de la semana (lunes a domingo)
  ...
  useEffect(() => {
    ...
    fetch(`/api/stats?startDate=${from}&endDate=${to}`)
      .then((res) => res.json())
      .then((data) => setStats(data))
      .finally(() => setLoading(false));
  }, [range]);

  // Cargar notas de venta del rango
  useEffect(() => {
    if (!range.from || !range.to) return;
    ...
    fetch(
      `/api/salenote?startDate=${from}&endDate=${to}&page=${page}&pageSize=${pageSize}`
    )
      .then((res) => res.json())
      .then((data) => {
      ...
      <div className="max-w-7xl mx-auto">
        <div className="mb-6">
          ...
          <p className="text-gray-600 mt-1">
            Selecciona cualquier día y automáticamente se mostrarán las estadísticas de toda la semana (lunes a domingo).
          </p>
        </div>
              ...
              <div className="mt-3 p-3 bg-blue-50 rounded-lg">
                <div className="text-center">
                  ...
                  <div className="text-sm font-bold text-blue-800">
                    Lunes {dayjs(range.from).format("DD/MM/YYYY")} al Domingo {dayjs(range.to).format("DD/MM/YYYY")}
                  </div>
                  <div className="text-xs text-blue-600 mt-1">
                    Semana #{dayjs(selectedDate).week()} del año {dayjs(selectedDate).year()}
                  </div>
                </div>
              </div>
            </div>
        ...
        {/* Tabla de notas de venta de la semana */}
        <div className="mt-8">
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            <div className="p-6 border-b border-gray-200 flex items-center justify-between">
              <h2 className="text-xl font-bold text-gray-800 flex items-center gap-3">
                <CreditCard className="text-blue-600" size={24} />
                Notas de venta de la semana
              </h2>
              {/* Paginación */}
              ...
              <table className="w-full">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {loadingNotes ? (
                    ...
                    <tr>
                      <td colSpan={10} className="text-center py-6 text-gray-500">
                        No hay notas de venta para esta semana.
                      </td>
                    </tr>
                  ) : (
                    saleNotes.map((note) => (
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
              <div className="p-6 border-b border-gray-200">
                <h2 className="text-xl font-bold text-gray-800 flex items-center gap-3">
                  ...
                  Detalle de productos vendidos en la semana
                </h2>
              </div>
